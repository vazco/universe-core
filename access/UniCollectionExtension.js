...
var _runHandler = function(handlerName, collectionName, id, doc, directSub){
    ...
    if(_.contains(['added', 'changed'], handlerName)){
        _doMapping(id, doc, directSub._uniMappings, directSub);
...
var _doMapping = function(id, doc, mappings, sub) {
    ...
    _.each(mappings, function(mapping){
        ...
        if (mapping.mappings) {
            if(!_.isArray(mapping.mappings)){
                throw new Meteor.Error(500, 'Property mappings must be an array');
            }
            var oldMappings = sub._uniMappings;
            sub._uniMappings = mapping.mappings;
            _eachCursorsCheck(mapping.collection.find(mapFilter, mapping.options||{}), sub, id);
            sub._uniMappings = oldMappings;
        } else {
            _eachCursorsCheck(mapping.collection.find(mapFilter, mapping.options), sub, id);
        }
    });
